...
export function AIIllustration() {
	...
	return (
		<View className="items-center justify-center">
			<Animated.View style={entryStyle} className="relative items-center justify-center">
				{/* Centro */}
				<Animated.View className="rounded-full bg-linear-to-br from-indigo-900 to-pink-600 p-10 shadow-2xl">
...
function OrbitDot({ index }: { index: number }) {
	...
	return (
		<Animated.View style={orbitStyle} className="absolute">
			<Animated.View
				style={pulseStyle}
				className="absolute left-1/2 top-0 h-4 w-4 -translate-x-1/2 rounded-full bg-linear-to-br from-indigo-900 to-pink-600 shadow-lg"
			/>
...
function FloatingParticle({ delay }: { delay: number }) {
	...
	useEffect(() => {
		y.value = withRepeat(
			withSequence(withTiming(-20, { duration: 1500 }), withTiming(0, { duration: 1500 })),
			-1,
			true,
		);
