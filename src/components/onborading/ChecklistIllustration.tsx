...
export function ChecklistIllustration() {
	...
	return (
		<View className="items-center">
					...
					<View key={index} className="mb-4 flex-row items-center gap-3">
						<View
							className={`h-6 w-6 items-center justify-center rounded-lg ${
								index < 2 ? "bg-linear-to-br from-indigo-900 to-pink-600" : "border-2 border-gray-300"
							}`}
						>
							{index < 2 && <CheckSquare size={14} color="white" />}
						</View>
						<View className={`h-3 flex-1 rounded-full ${index < 2 ? "bg-gray-200" : "bg-gray-100"}`} />
					</View>
				))}
			</View>
			...
			<Animated.View
				style={floatStyle}
				className="absolute -right-4 -top-4 rounded-2xl bg-linear-to-br from-indigo-900 to-pink-600 p-3 shadow-lg"
			>
